...
def similar_odds_matches(df: pd.DataFrame, home_prob: float, draw_prob: float, away_prob: float, tolerance: float = 0.04) -> pd.DataFrame:
    """Find historical rows with similar implied-probability profiles as supporting context."""
    valid = df.dropna(subset=PROB_COLS + ["FTR"]).copy()
    if valid.empty:
        valid["OddsDistance"] = pd.Series(dtype="float64")
        return valid
...
def similar_odds_summary(matches: pd.DataFrame) -> dict[str, float | int]:
    """Summarize outcome frequencies for similar historical odds profiles."""
    if matches.empty:
        return {"matches": 0, "home_rate": 0.0, "draw_rate": 0.0, "away_rate": 0.0}
    rates = matches["FTR"].value_counts(normalize=True)
    return {
        "matches": int(len(matches)),
        "home_rate": float(rates.get("H", 0.0)),
        "draw_rate": float(rates.get("D", 0.0)),
        "away_rate": float(rates.get("A", 0.0)),
    }
