...
from src.config import load_competitions
from src.data_loader import load_international_files, load_uploaded_files
...
with st.sidebar:
    ...
    competition = st.selectbox("Competition", [item["name"] for item in competitions])
    competition_meta = next(item for item in competitions if item["name"] == competition)
    uploads = st.file_uploader("Upload match CSV files", type="csv", accept_multiple_files=True)
    st.caption(f"Source: {competition_meta['data_source']} • Type: {competition_meta['match_type']}")
...
if not uploads:
    st.info("Upload one or more football-data.co.uk or international match CSV files to begin.")
    st.markdown(
        """
...
try:
    data = load_international_files(uploads) if competition_meta["match_type"] == "international" else load_uploaded_files(uploads, match_type=competition_meta["match_type"])
...
if page == "Overview":
    ...
    st.write("This modular baseline combines recent form, venue performance, goal trends, head-to-head history, Elo ratings, bookmaker implied probabilities, and similar historical matches as supporting context.")
    model, regressors, training_data = train_baseline_model(data)
    if model is None or regressors is None:
        st.warning("At least 50 feature-ready historical matches with valid odds are needed to train the baseline model.")
    ...
    result = predict_match(model, regressors, data, home_team, away_team, implied)
    prediction = result["probabilities"]
    home_score, away_score = result["predicted_score"]

    c6, c7, c8 = st.columns(3)
    c6.metric("Most likely result", result["most_likely"])
    c7.metric("Predicted score", f"{home_score:.1f} - {away_score:.1f}")
    c8.metric("Confidence", f"{result['confidence']:.0%}")
    ...
    with st.expander("Supporting context: similar historical odds matches"):
        st.write(result["similar_summary"])
        st.dataframe(result["similar_matches"][["Date", "HomeTeam", "AwayTeam", "FTHG", "FTAG", "FTR", "ImpHome", "ImpDraw", "ImpAway", "OddsDistance"]].head(30), use_container_width=True)
    with st.expander("Engineered feature row"):
